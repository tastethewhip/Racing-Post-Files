...
class JSONToCSVConverter:
    def __init__(self, root):
        ...
        self.column_order = [
            ...
            'horse_name', 'field_size', 'draw', 'age', 'lbs', 'form',
            'jockey', 'owner', 'race_class', 'rpr', 'sex', 'sire', 'dam',
            'spotlight', 'comment', 'trainer', 'trainer_14_days', 'trainer_rtf',
            'ts', 'stable_tour', 'stats', 'medical'
        ]
    ...
    def convert_files(self):
        ...
        for file_path in self.files:
            try:
                ...
                # Build headers: use predefined order, then add any extra columns not in the order
                headers = self.column_order.copy()
                for row in rows:
                    for key in row.keys():
